...
import { getAssets, getAttendance, getHelpdeskTickets, getNotifications } from "../services/api";
import { formatDate } from "../utils/employeeStats";

const leaveEntitlements = { paid_leave: 14, sick_leave: 7 };
...
function userInScope(currentUser, user) {
  if (!currentUser || !user) {
    return false;
  }
  ...
  if (currentUser.role === "manager") {
    return user.email === currentUser.email || user.managerEmail === currentUser.email;
  }
...
function KpiCard({ color, icon: Icon, label, sublabel, value }) {
  return (
    <article className="relative min-h-[116px] overflow-hidden rounded-2xl p-5 text-white shadow-lg shadow-slate-300/70">
      <div className="absolute inset-0" style={{ backgroundColor: color }} />
      <div className="absolute -right-9 -top-10 h-28 w-28 rounded-full bg-white/10" />
      <div className="absolute -bottom-8 right-0 h-24 w-28 rounded-tl-full bg-white/10" />
      <div className="relative flex h-full items-start justify-between gap-3">
        <div>
          <p className="text-sm font-black text-white/85">{label}</p>
          <h3 className="mt-3 text-3xl font-black leading-none">{value}</h3>
          <p className="mt-3 text-xs font-bold text-white/75">{sublabel}</p>
        </div>
...
function Panel({ action, children, className = "", title, subtitle }) {
  return (
    <section className={`overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg shadow-slate-200/70 ${className}`}>
      <div className="flex items-start justify-between gap-3 border-b border-slate-100 px-5 py-4">
        <div>
          <h2 className="text-lg font-black text-[#15372b]">{title}</h2>
          {subtitle ? <p className="mt-1 text-xs font-semibold text-slate-500">{subtitle}</p> : null}
        </div>
        {action ? (
          <button className="inline-flex shrink-0 items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-[#064b36] hover:bg-[#eff6df]" type="button">
            {action}
            <ArrowRight size={14} />
          </button>
        ) : null}
...
function ReportsAnalytics({ currentUser, users = [] }) {
  ...
  async function loadReports() {
    ...
    try {
      const [attendanceData, assetData, ticketData, notificationData] = await Promise.all([
        getAttendance(),
        getAssets(),
        getHelpdeskTickets(),
        getNotifications()
  ...
  const scopedUsers = useMemo(() => users.filter((user) => userInScope(currentUser, user)), [currentUser, users]);
  const today = new Date();
  const todayLabel = today.toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric", weekday: "long" });
  const activeUsers = scopedUsers.filter((user) => user.status === "active");
  const newJoiners = scopedUsers.filter((user) => {
  ...
  const leaveUsed = attendance.reduce(
    (total, record) => {
      if (new Date(record.date).getFullYear() !== today.getFullYear()) {
        return total;
      }
  ...
  const leaveBalance = {
    paid: Math.max(0, activeUsers.length * leaveEntitlements.paid_leave - leaveUsed.paid),
    sick: Math.max(0, activeUsers.length * leaveEntitlements.sick_leave - leaveUsed.sick)
  };
  const openTickets = tickets.filter((ticket) => ["open", "in_progress"].includes(ticket.status)).length;
  const birthdayToday = scopedUsers.filter((user) => {
    if (!user.dateOfBirth) {
      return false;
    }
  ...
  const upcomingBirthdays = scopedUsers
  ...
  const attendanceTotal = Math.max(1, attendanceMix.reduce((total, [, value]) => total + value, 0));
  const presentPercent = Math.round((presentCount / attendanceTotal) * 100);
  const assetRows = [
    ["Laptop", assets.filter((asset) => asset.category === "laptop").length, Monitor],
    ["Desktop", assets.filter((asset) => asset.category === "desktop").length, Monitor],
    ["Accessories", assets.filter((asset) => asset.category === "accessory").length, Package],
    ["Mouse", assets.filter((asset) => asset.category === "mouse").length, Mouse],
    ["Other Assets", assets.filter((asset) => asset.category === "other").length, Package]
  ];
  const ticketRows = [
  ...
  const rootPerson =
    currentUser.role === "manager"
      ? scopedUsers.find((user) => user.email === currentUser.email) || currentUser
      : scopedUsers.find((user) => ["admin", "hr"].includes(user.role)) || scopedUsers.find((user) => user.role === "manager") || scopedUsers[0];
  const directReports = rootPerson ? scopedUsers.filter((user) => user.managerEmail === rootPerson.email).slice(0, 4) : [];

  return (
    <section className="space-y-5">
      <div className="relative min-h-[140px] overflow-hidden rounded-2xl bg-[#064b36] px-8 py-7 text-white shadow-xl shadow-emerald-900/20">
        <div className="absolute right-0 top-0 h-full w-2/5 bg-[radial-gradient(circle_at_top_right,rgba(191,255,47,0.24),transparent_55%)]" />
        <div className="absolute right-8 top-3 hidden text-white/20 lg:block">
          <UsersRound size={118} strokeWidth={1.2} />
        </div>
        <div className="relative flex h-full flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-3xl font-black sm:text-4xl">AAGarg HR Command Center</h1>
            <p className="mt-3 max-w-2xl text-sm font-semibold text-emerald-50/80">Your central hub for people, operations, and workplace excellence.</p>
          </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <KpiCard color="#123c69" icon={CalendarDays} label="Leave Balance" sublabel="Paid Leave / Sick Leave" value={`${leaveBalance.paid} / ${leaveBalance.sick}`} />
      ...
      <div className="grid gap-5 xl:grid-cols-12">
        <Panel action="View All" className="xl:col-span-5" title="Employee Directory" subtitle="Latest people in your visible scope.">
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {scopedUsers.slice(0, 5).map((user) => (
                  <tr key={user.id} className="hover:bg-[#eff6df]">
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="whitespace-nowrap font-black text-[#15372b]">{user.name}</p>
                    ...
                    <td className="whitespace-nowrap px-5 py-4 text-slate-600">{formatDate(user.joinedAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Panel className="xl:col-span-4" title="Attendance Summary" subtitle="Today's attendance overview">
          <div className="grid min-h-[245px] gap-5 p-5 sm:grid-cols-[0.9fr_1fr] sm:items-center">
            <div className="mx-auto flex h-40 w-40 items-center justify-center rounded-full" style={{ background: `conic-gradient(#66bb5a 0 ${presentPercent}%, #e5e7eb ${presentPercent}% 100%)` }}>
        ...
        <Panel action="View All" className="xl:col-span-3" title="Upcoming Birthdays">
          <div className="divide-y divide-slate-100">
            ...
            {!upcomingBirthdays.length ? <p className="px-5 py-10 text-center text-sm font-bold text-slate-500">No upcoming birthdays in your scope.</p> : null}
          </div>
        </Panel>
      </div>

      <div className="grid gap-5 xl:grid-cols-12">
        <Panel action="View Org Chart" className="xl:col-span-4" title="Organization Structure" subtitle="Your visible reporting structure">
          <div className="min-h-[260px] p-5">
            {rootPerson ? (
              <div className="flex flex-col items-center">
                <div className="rounded-2xl bg-[#eff6df] px-8 py-4 text-center shadow-sm">
                  <p className="font-black text-[#15372b]">{rootPerson.name}</p>
                  <p className="text-xs text-slate-500">{rootPerson.designation || rootPerson.role}</p>
                </div>
                ...
                ) : <p className="mt-8 text-sm font-bold text-slate-500">No reporting members in this view.</p>}
              </div>
            ) : <p className="text-sm font-bold text-slate-500">No organization data yet.</p>}
          </div>
        </Panel>

        <Panel action="View All" className="xl:col-span-3" title="Asset Register" subtitle="Assets in your visible scope">
          <div className="divide-y divide-slate-100">
            {assetRows.map(([label, count, Icon]) => (
              <div key={label} className="flex items-center justify-between px-5 py-4">
                <span className="inline-flex items-center gap-3 text-sm font-black text-[#15372b]"><Icon size={18} className="text-[#064b36]" />{label}</span>
                <span className="font-black text-[#064b36]">{count}</span>
              </div>
            ))}
            <div className="flex items-center justify-between bg-slate-50 px-5 py-4 text-sm font-black text-[#15372b]">
        ...
        <Panel action="View All" className="xl:col-span-2" title="Ticket Desk" subtitle="Helpdesk ticket summary">
          <div className="divide-y divide-slate-100">
        ...
        <Panel action="View All" className="xl:col-span-3" title="Recent Notifications">
          <div className="divide-y divide-slate-100">
            {notifications.slice(0, 5).map((notification) => (
              <div key={notification.id} className="flex items-center gap-3 px-5 py-4">
                ...
                <p className="min-w-0 flex-1 truncate text-sm font-bold text-slate-600">{notification.message}</p>
